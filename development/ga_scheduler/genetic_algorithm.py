...
class Job:
    ...
    def get_min_completion_time(self):
        ...
        if self.min_completion_time < 0:
            self.min_completion_time = self.burst_time
...
# Schedule Encapulating Class
class Schedule:
    def __init__(self, processor_schedules=None):
        """
            Initializes the schedule with the appropriate information
        """
    ...
    def get_task_location(self, task):
        ...
        for i in range(len(self.processor_schedules)):
            for j in range(len(self.processor_schedules[i])):
                if self.processor_schedules[i][j].id == task.id:
                    return i, j
        return None
    ...
    def calculate_task_completion(self, processor_index, task_index):
        ...
        if task in self.task_completion_map:
            return self.task_completion_map[task.id]
        ...
        self.task_completion_map[task.id] = max(previous_task_completion,
                                                        max(dependency_completions + [0])) + task.burst_time
        return self.task_completion_map[task.id]
    ...
    def calculate_time_grid(self, total_time):
        ...
        for i, processor in enumerate(self.processor_schedules):
            for j, task in enumerate(processor):
                ...
                time_grid[i][(end_time - task.burst_time):end_time] = [task] * task.burst_time
...
class GeneticTaskScheduler:
    def __init__(self, tasks):
        """
            Initializes the wrapper class with the appropriate information
        """
        self.tasks = tasks
        self.total_time = 0
    ...
    def initialize(self, num_processors, population_size, total_time):
        ...
        for task in prioritized_tasks:
            self.total_time_bound += task.burst_time
